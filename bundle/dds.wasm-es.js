var YA = (() => {
  var u = import.meta.url;
  return function(A) {
    A = A || {};
    var A = typeof A < "u" ? A : {}, h, a;
    A.ready = new Promise(function(I, g) {
      h = I, a = g;
    });
    var w = Object.assign({}, A), y = typeof window == "object", R = typeof importScripts == "function", l = typeof process == "object" && typeof process.versions == "object" && typeof process.versions.node == "string", o = "";
    function EA(I) {
      return A.locateFile ? A.locateFile(I, o) : o + I;
    }
    var K, H, Y;
    if (l) {
      R ? o = require("path").dirname(o) + "/" : o = __dirname + "/";
      var b, m, _ = () => {
        m || (b = require("fs"), m = require("path"));
      };
      K = (I, g) => (_(), I = m.normalize(I), b.readFileSync(I, g ? void 0 : "utf8")), Y = (I) => {
        var g = K(I, !0);
        return g.buffer || (g = new Uint8Array(g)), g;
      }, H = (I, g, C) => {
        _(), I = m.normalize(I), b.readFile(I, function(E, e) {
          E ? C(E) : g(e.buffer);
        });
      }, process.argv.length > 1 && process.argv[1].replace(/\\/g, "/"), process.argv.slice(2), process.on("uncaughtException", function(I) {
        if (!(I instanceof yA))
          throw I;
      }), process.on("unhandledRejection", function(I) {
        throw I;
      }), A.inspect = function() {
        return "[Emscripten Module object]";
      };
    } else
      (y || R) && (R ? o = self.location.href : typeof document < "u" && document.currentScript && (o = document.currentScript.src), u && (o = u), o.indexOf("blob:") !== 0 ? o = o.substr(0, o.replace(/[?#].*/, "").lastIndexOf("/") + 1) : o = "", K = (I) => {
        var g = new XMLHttpRequest();
        return g.open("GET", I, !1), g.send(null), g.responseText;
      }, R && (Y = (I) => {
        var g = new XMLHttpRequest();
        return g.open("GET", I, !1), g.responseType = "arraybuffer", g.send(null), new Uint8Array(g.response);
      }), H = (I, g, C) => {
        var E = new XMLHttpRequest();
        E.open("GET", I, !0), E.responseType = "arraybuffer", E.onload = () => {
          if (E.status == 200 || E.status == 0 && E.response) {
            g(E.response);
            return;
          }
          C();
        }, E.onerror = C, E.send(null);
      });
    A.print || console.log.bind(console);
    var d = A.printErr || console.warn.bind(console);
    Object.assign(A, w), w = null, A.arguments && A.arguments, A.thisProgram && A.thisProgram, A.quit && A.quit;
    var k;
    A.wasmBinary && (k = A.wasmBinary), A.noExitRuntime, typeof WebAssembly != "object" && J("no native wasm support detected");
    var p, x = !1, X = typeof TextDecoder < "u" ? new TextDecoder("utf8") : void 0;
    function iA(I, g, C) {
      for (var E = g + C, e = g; I[e] && !(e >= E); )
        ++e;
      if (e - g > 16 && I.buffer && X)
        return X.decode(I.subarray(g, e));
      for (var i = ""; g < e; ) {
        var B = I[g++];
        if (!(B & 128)) {
          i += String.fromCharCode(B);
          continue;
        }
        var Q = I[g++] & 63;
        if ((B & 224) == 192) {
          i += String.fromCharCode((B & 31) << 6 | Q);
          continue;
        }
        var t = I[g++] & 63;
        if ((B & 240) == 224 ? B = (B & 15) << 12 | Q << 6 | t : B = (B & 7) << 18 | Q << 12 | t << 6 | I[g++] & 63, B < 65536)
          i += String.fromCharCode(B);
        else {
          var c = B - 65536;
          i += String.fromCharCode(55296 | c >> 10, 56320 | c & 1023);
        }
      }
      return i;
    }
    function eA(I, g) {
      return I ? iA(U, I, g) : "";
    }
    function nA(I, g, C, E) {
      if (!(E > 0))
        return 0;
      for (var e = C, i = C + E - 1, B = 0; B < I.length; ++B) {
        var Q = I.charCodeAt(B);
        if (Q >= 55296 && Q <= 57343) {
          var t = I.charCodeAt(++B);
          Q = 65536 + ((Q & 1023) << 10) | t & 1023;
        }
        if (Q <= 127) {
          if (C >= i)
            break;
          g[C++] = Q;
        } else if (Q <= 2047) {
          if (C + 1 >= i)
            break;
          g[C++] = 192 | Q >> 6, g[C++] = 128 | Q & 63;
        } else if (Q <= 65535) {
          if (C + 2 >= i)
            break;
          g[C++] = 224 | Q >> 12, g[C++] = 128 | Q >> 6 & 63, g[C++] = 128 | Q & 63;
        } else {
          if (C + 3 >= i)
            break;
          g[C++] = 240 | Q >> 18, g[C++] = 128 | Q >> 12 & 63, g[C++] = 128 | Q >> 6 & 63, g[C++] = 128 | Q & 63;
        }
      }
      return g[C] = 0, C - e;
    }
    function tA(I, g, C) {
      return nA(I, U, g, C);
    }
    var Z, T, U;
    function P(I) {
      Z = I, A.HEAP8 = T = new Int8Array(I), A.HEAP16 = new Int16Array(I), A.HEAP32 = new Int32Array(I), A.HEAPU8 = U = new Uint8Array(I), A.HEAPU16 = new Uint16Array(I), A.HEAPU32 = new Uint32Array(I), A.HEAPF32 = new Float32Array(I), A.HEAPF64 = new Float64Array(I);
    }
    A.INITIAL_MEMORY;
    var j = [], W = [], O = [];
    function rA() {
      if (A.preRun)
        for (typeof A.preRun == "function" && (A.preRun = [A.preRun]); A.preRun.length; )
          sA(A.preRun.shift());
      L(j);
    }
    function aA() {
      L(W);
    }
    function oA() {
      if (A.postRun)
        for (typeof A.postRun == "function" && (A.postRun = [A.postRun]); A.postRun.length; )
          DA(A.postRun.shift());
      L(O);
    }
    function sA(I) {
      j.unshift(I);
    }
    function cA(I) {
      W.unshift(I);
    }
    function DA(I) {
      O.unshift(I);
    }
    var F = 0, G = null;
    function fA(I) {
      F++, A.monitorRunDependencies && A.monitorRunDependencies(F);
    }
    function RA(I) {
      if (F--, A.monitorRunDependencies && A.monitorRunDependencies(F), F == 0 && G) {
        var g = G;
        G = null, g();
      }
    }
    function J(I) {
      A.onAbort && A.onAbort(I), I = "Aborted(" + I + ")", d(I), x = !0, I += ". Build with -sASSERTIONS for more info.";
      var g = new WebAssembly.RuntimeError(I);
      throw a(g), g;
    }
    var uA = "data:application/octet-stream;base64,";
    function V(I) {
      return I.startsWith(uA);
    }
    function z(I) {
      return I.startsWith("file://");
    }
    var n;
    A.locateFile ? (n = "dds.wasm", V(n) || (n = EA(n))) : n = new URL("data:application/wasm;base64,", self.location).toString();
    function $(I) {
      try {
        if (I == n && k)
          return new Uint8Array(k);
        if (Y)
          return Y(I);
        throw "both async and sync fetching of the wasm failed";
      } catch (g) {
        J(g);
      }
    }
    function hA() {
      if (!k && (y || R)) {
        if (typeof fetch == "function" && !z(n))
          return fetch(n, { credentials: "same-origin" }).then(function(I) {
            if (!I.ok)
              throw "failed to load wasm binary file at '" + n + "'";
            return I.arrayBuffer();
          }).catch(function() {
            return $(n);
          });
        if (H)
          return new Promise(function(I, g) {
            H(n, function(C) {
              I(new Uint8Array(C));
            }, g);
          });
      }
      return Promise.resolve().then(function() {
        return $(n);
      });
    }
    function wA() {
      var I = { a: GA };
      function g(B, Q) {
        var t = B.exports;
        A.asm = t, p = A.asm.c, P(p.buffer), A.asm.m, cA(A.asm.d), RA();
      }
      fA();
      function C(B) {
        g(B.instance);
      }
      function E(B) {
        return hA().then(function(Q) {
          return WebAssembly.instantiate(Q, I);
        }).then(function(Q) {
          return Q;
        }).then(B, function(Q) {
          d("failed to asynchronously prepare wasm: " + Q), J(Q);
        });
      }
      function e() {
        return !k && typeof WebAssembly.instantiateStreaming == "function" && !V(n) && !z(n) && !l && typeof fetch == "function" ? fetch(n, { credentials: "same-origin" }).then(function(B) {
          var Q = WebAssembly.instantiateStreaming(B, I);
          return Q.then(C, function(t) {
            return d("wasm streaming compile failed: " + t), d("falling back to ArrayBuffer instantiation"), E(C);
          });
        }) : E(C);
      }
      if (A.instantiateWasm)
        try {
          var i = A.instantiateWasm(I, g);
          return i;
        } catch (B) {
          d("Module.instantiateWasm callback failed with error: " + B), a(B);
        }
      return e().catch(a), {};
    }
    function yA(I) {
      this.name = "ExitStatus", this.message = "Program terminated with exit(" + I + ")", this.status = I;
    }
    function L(I) {
      for (; I.length > 0; )
        I.shift()(A);
    }
    function FA(I, g) {
      T.set(I, g);
    }
    function NA(I, g, C) {
      U.copyWithin(I, g, g + C);
    }
    function lA() {
      return 2147483648;
    }
    function SA(I) {
      try {
        return p.grow(I - Z.byteLength + 65535 >>> 16), P(p.buffer), 1;
      } catch {
      }
    }
    function dA(I) {
      var g = U.length;
      I = I >>> 0;
      var C = lA();
      if (I > C)
        return !1;
      let E = (t, c) => t + (c - t % c) % c;
      for (var e = 1; e <= 4; e *= 2) {
        var i = g * (1 + 0.2 / e);
        i = Math.min(i, I + 100663296);
        var B = Math.min(C, E(Math.max(I, i), 65536)), Q = SA(B);
        if (Q)
          return !0;
      }
      return !1;
    }
    function AA(I) {
      var g = A["_" + I];
      return g;
    }
    function kA(I, g, C, E, e) {
      var i = { string: (r) => {
        var S = 0;
        if (r != null && r !== 0) {
          var QA = (r.length << 2) + 1;
          S = q(QA), tA(r, S, QA);
        }
        return S;
      }, array: (r) => {
        var S = q(r.length);
        return FA(r, S), S;
      } };
      function B(r) {
        return g === "string" ? eA(r) : g === "boolean" ? Boolean(r) : r;
      }
      var Q = AA(I), t = [], c = 0;
      if (E)
        for (var N = 0; N < E.length; N++) {
          var BA = i[C[N]];
          BA ? (c === 0 && (c = IA()), t[N] = BA(E[N])) : t[N] = E[N];
        }
      var v = Q.apply(null, t);
      function HA(r) {
        return c !== 0 && gA(c), B(r);
      }
      return v = HA(v), v;
    }
    function UA(I, g, C, E) {
      C = C || [];
      var e = C.every((B) => B === "number" || B === "boolean"), i = g !== "string";
      return i && e && !E ? AA(I) : function() {
        return kA(I, g, C, arguments);
      };
    }
    var GA = { b: NA, a: dA };
    wA(), A.___wasm_call_ctors = function() {
      return (A.___wasm_call_ctors = A.asm.d).apply(null, arguments);
    }, A._call_load_wrapper = function() {
      return (A._call_load_wrapper = A.asm.e).apply(null, arguments);
    }, A._create_buffer = function() {
      return (A._create_buffer = A.asm.f).apply(null, arguments);
    }, A._destroy_buffer = function() {
      return (A._destroy_buffer = A.asm.g).apply(null, arguments);
    }, A._get_pointer = function() {
      return (A._get_pointer = A.asm.h).apply(null, arguments);
    }, A._get_size = function() {
      return (A._get_size = A.asm.i).apply(null, arguments);
    }, A._get_width = function() {
      return (A._get_width = A.asm.j).apply(null, arguments);
    }, A._get_height = function() {
      return (A._get_height = A.asm.k).apply(null, arguments);
    }, A._get_format_flags = function() {
      return (A._get_format_flags = A.asm.l).apply(null, arguments);
    };
    var IA = A.stackSave = function() {
      return (IA = A.stackSave = A.asm.n).apply(null, arguments);
    }, gA = A.stackRestore = function() {
      return (gA = A.stackRestore = A.asm.o).apply(null, arguments);
    }, q = A.stackAlloc = function() {
      return (q = A.stackAlloc = A.asm.p).apply(null, arguments);
    };
    A.cwrap = UA;
    var M;
    G = function I() {
      M || CA(), M || (G = I);
    };
    function CA(I) {
      if (F > 0 || (rA(), F > 0))
        return;
      function g() {
        M || (M = !0, A.calledRun = !0, !x && (aA(), h(A), A.onRuntimeInitialized && A.onRuntimeInitialized(), oA()));
      }
      A.setStatus ? (A.setStatus("Running..."), setTimeout(function() {
        setTimeout(function() {
          A.setStatus("");
        }, 1), g();
      }, 1)) : g();
    }
    if (A.preInit)
      for (typeof A.preInit == "function" && (A.preInit = [A.preInit]); A.preInit.length > 0; )
        A.preInit.pop()();
    return CA(), A.ready;
  };
})();
let s, D;
const pA = new Promise((u) => {
  YA().then(
    async (f) => {
      D = f, s = {
        alloc: D.cwrap("create_buffer", "number", ["number"]),
        load: D.cwrap("call_load_wrapper", "", ["array", "number"]),
        getPointer: D.cwrap("get_pointer", "number", []),
        getSize: D.cwrap("get_size", "number", []),
        getWidth: D.cwrap("get_width", "number", []),
        getHeight: D.cwrap("get_height", "number", []),
        getFlags: D.cwrap("get_format_flags", "number", []),
        clean: D.cwrap("destroy_buffer", "", ["number"])
      }, u();
    }
  );
});
async function MA(u, f) {
  f = { outputFormat: "url", transparency: !1, ...f };
  let A = await fetch(u);
  A = await A.arrayBuffer(), A = new Uint8Array(A);
  const h = s.alloc(A.length);
  D.HEAPU8.set(A, h), s.load(h, A.length);
  const a = { width: s.getWidth(), height: s.getHeight(), flags: s.getFlags() };
  let w = new Uint8Array(
    D.HEAPU8.buffer,
    s.getPointer(),
    s.getSize()
  );
  f.transparency || (w = w.map((R, l) => l > 0 && l % 4 === 3 ? 255 : R));
  const y = await mA(w, a.width, a.height, s.getSize());
  return s.clean(h), s.clean(s.getPointer()), y ? f.outputFormat === "url" ? URL.createObjectURL(y) : { blob: y, ...a } : !1;
}
async function mA(u, f, A, h) {
  if (!f)
    return !1;
  const a = document.createElement("canvas");
  a.width = f, a.height = A;
  const w = a.getContext("2d"), y = new ImageData(new Uint8ClampedArray(u.slice(0, h)), f, A);
  return w.drawImage(await createImageBitmap(y, { imageOrientation: "flipY" }), 0, 0), new Promise((R, l) => a.toBlob(R));
}
export {
  MA as getDDSImage,
  pA as isReady
};
